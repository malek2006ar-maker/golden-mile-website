import {
  TrendingUp, TrendingDown, FolderKanban, Users, DollarSign,
...
export function StatCard({ title, value, change, changeLabel, icon, color = "gold" }: StatCardProps) {
  ...
  return (
    <div className={cn(
      "relative overflow-hidden rounded-2xl p-5 bg-ink-800/60",
      "border border-white/5 hover:border-gold-600/30",
      "transition-all duration-300 group card-hover"
    )}>
      <div className={cn(
        "absolute -top-10 -left-10 w-32 h-32 rounded-full opacity-20 blur-2xl",
      ...
      <div className="relative flex items-start justify-between mb-4">
        <div className={cn(
          ...
          "bg-gradient-to-br ring-1", c.bg, c.ring
        )}>
      ...
      <div className="relative">
        ...
        <p className="text-2xl sm:text-3xl font-extrabold text-white font-tajawal">
          {value}
        </p>
        {changeLabel && (
          <p className="text-[11px] text-slate-500 mt-2">{changeLabel}</p>
        )}
      </div>
    </div>
  );
