...
import { Plus, Download, Eye, Reply, Phone, Mail, Inbox } from "lucide-react";
import { getStatusBadge, getPriorityBadge, timeAgo, formatDateTime } from "@/lib/utils";

const sourceLabel: Record<string, string> = {
  contact_form: "نموذج التواصل", design_studio: "استوديو التصميم", store: "المتجر", phone: "هاتفي",
};
...
export default async function InquiriesPage() {
  ...
  const columns: Column<any>[] = [
    {
      key: "name", label: "المرسل", sortable: true,
      render: (i) => (
        <div className="flex items-center gap-3">
          <Avatar name={i.name} size="md" />
          <div className="min-w-0">
            <p className="font-bold text-white truncate">{i.name}</p>
            <p className="text-[11px] text-slate-500 truncate" dir="ltr">{i.phone}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (i) => (
        <div className="max-w-xs">
          <p className="text-sm font-semibold text-white truncate">{i.subject}</p>
          <p className="text-[11px] text-slate-500 line-clamp-1">{i.message}</p>
        </div>
      ),
    },
    { key: "source", label: "المصدر", sortable: true, render: (i) => <span className="badge badge-neutral">{sourceLabel[i.source] || i.source}</span> },
    ...
    { key: "createdAt", label: "التاريخ", sortable: true, render: (i) => <div><p className="text-xs text-slate-300">{formatDateTime(i.createdAt)}</p><p className="text-[10px] text-slate-500">{timeAgo(i.createdAt)}</p></div> },
    {
      ...
      render: () => (
        <div className="flex items-center gap-1 justify-end">
          ...
          <button className="p-1.5 rounded-lg text-slate-400 hover:text-gold-300 hover:bg-gold-600/10 transition-colors"><Reply className="w-4 h-4" /></button>
  ...
  return (
    <div>
      <PageHeader
        title="الاستفسارات"
        description="إدارة استفسارات العملاء من الموقع والتواصل"
        breadcrumbs={[{ label: "لوحة التحكم", href: "/dashboard" }, { label: "الاستفسارات" }]}
        actions={<><Button variant="outline" size="sm"><Download className="w-4 h-4" /><span>تصدير</span></Button><Button size="sm"><Plus className="w-4 h-4" /><span>استفسار يدوي</span></Button></>}
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div className="card p-4"><p className="text-[11px] text-slate-500 mb-1">أولوية عالية</p><p className="text-2xl font-extrabold text-danger">{highPriorityCount}</p></div>
